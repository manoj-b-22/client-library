...
const Profile = ({ user }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      await axios
        .get(
          `https://radiant-woodland-38466.herokuapp.com/search/orders?username=${user.username}&return=false`,
          {
            headers: {
              "x-access-token": user.token,
            },
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      await axios
        .get(`https://radiant-woodland-38466.herokuapp.com/search/books?addedBy=${user.username}`, {
          headers: {
            "x-access-token": user.token,
          },
        })
